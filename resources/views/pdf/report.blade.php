...
<head>
    ...
    <style>
        ...
        .company-logo { float: right; max-height: 60px; }
        ...
        .section-title { font-size: 14px; font-weight: bold; background: #f3f4f6; padding: 5px; border-left: 4px solid #3b82f6; width: 100%; display: block; }
        ...
        .checked { background: #3b82f6; color: white; border-color: #3b82f6; }
        ...
        <table class="grid">
            <tr>
                ...
                <td width="50%">
                    ...
                    <div>Status: {{ $job->status->value }}</div>
                    <div>Data wykonania: {{ $job->scheduled_at->format('d.m.Y') }}</div>
                    <div>Technik: {{ $job->technician->name }}</div>
                </td>
            </tr>
        </table>
    </div>
